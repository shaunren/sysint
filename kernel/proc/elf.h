...
struct Elf32_Ehdr
{
  ...
  Elf32_Half	e_shstrndx;		/* Section header string table index */
    ...
    inline const Elf32_Shdr* get_shdr() const
    {
        if (!e_shoff)
            return nullptr;
        return (const Elf32_Shdr*) ((const char*)this + e_shoff);
    }

    inline const Elf32_Phdr* get_phdr() const
    {
        if (!e_phoff)
            return nullptr;
        return (const Elf32_Phdr*) ((const char*)this + e_phoff);
    }

    const char* strtab() const;
    const char* get_str(size_t offset) const;
};
...
// this function loads an elf executable in buf
// dir must contain the current kernel stack and buf
// this function will load dir in the process
int load(const void* buf, paging::page_dir* dir, Elf32_Addr& entry, void*& brk_start);
